...
async function getByName(name, entityName) {
  ...
  const item = await strapi.services[entityName].find({ slug })
...
async function createManyToManyData(products) {
  ...
  products.forEach(product => {
    ...
    genres?.forEach(item => categories.add(item))
    supportedOperatingSystems?.forEach(item => plataforms.add(item))
  ...
  return Promise.all([
    ...
    ...createCall(plataforms, "platform")
  ])
}
